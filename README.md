...
- **Phase 2B Step 3.1**: Completed (AI Intent Classification). A lightweight, zero-dependency LLM classifier (`LlmIntentClassifier`) built on `java.net.http.HttpClient` maps customer intent directly to `IntentTaxonomy`. 
    ...
    - **Configuration**: Properties include `supportiq.classifier.api-url`, `supportiq.classifier.api-key`, `supportiq.classifier.model` (default `gemini-2.5-flash`), and `supportiq.classifier.confidence-threshold` (default `0.6`). **Note:** Gemini is the default provider.
    - **Confidence**: Model-provided score (0.0 - 1.0); not a calibrated statistical probability.
    - **Uncertainty & Fallbacks**: The LLM prompt explicitly provides the exact allowed subcategories and strictly prohibits inventing labels. The classifier safely parses and validates the combination: if the category/subcategory mapping is invalid or confidence is too low, it automatically falls back to `GENERAL_INFORMATION_AND_NON_SUPPORT / UNKNOWN` with `uncertain=true` and `confidence=0.0`. Handles ambiguous/multilingual queries natively.
    - **Tests**: Unit tests do not require a live LLM API.
- **Phase 2B Step 3.2**: Completed (Historical Retrieval). Efficiently retrieves historical interactions for a specific category and subcategory without repeatedly scanning the full dataset.
    - **Data Partitioning (Real Labels)**: The `HistoricalMappingPreparer` loads historical customer texts and uses `LlmIntentClassifier` to assign them to their true, validated category and subcategory directories.
    - **Validation Sample**: The preparer runs deterministically on a limited validation sample (e.g., 500 records) to preserve API cost while proving the architecture. It emits an auditable `validation_sample.csv` containing the raw customer text, intent, and confidence score. The API key is provided via `GEMINI_API_KEY`. LLM predictions are provisional until human review. Staging mapping contains only valid, non-uncertain predictions.
...
- **Phase 2B Step 3.3**: Pending (Semantic Re-ranking). No vector databases or embeddings have been implemented yet.
